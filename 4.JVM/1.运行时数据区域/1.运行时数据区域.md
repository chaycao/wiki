...
![运行时数据区域](images/运行时数据区域.png)

## 程序计数器

记录正在执行的虚拟机字节码指令的地址（如果正在执行的是本地方法则为零）。

## 虚拟机栈

每个 Java 方法在执行的同时会创建一个栈帧用于存储**局部变量表**、**操作数栈**、**常量池引用**等信息。每一个方法从调用直至执行完成的过程，就对应着一个栈帧在 Java 虚拟机栈中入栈和出栈的过程。
...
![虚拟机栈](images\本地方法栈.gif)

## 堆

**所有对象实例都在这里分配内存**。

是垃圾收集的主要区域（"GC 堆"），现代的垃圾收集器基本都是采用**分代收集算法**，该算法的思想是针对不同的对象采取不同的垃圾回收算法，因此虚拟机把 Java 堆分成以下三块：
...
**Class 文件中的常量池**（编译器生成的各种**字面量**和**符号引用**）会在类加载后被放入这个区域。
